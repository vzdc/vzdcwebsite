...
@section('content')
    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-sm-2">
                <a class="btn btn-primary" href="/dashboard/admin/bronze-mic/<?=$pyr?>/<?=$pm?>"><i
                            class="fa fa-arrow-left"></i> Previous Month</a></li>
            </div>
            <div class="col-sm-8">
                <center><h4>Showing Eligible Hours for <?=$mname?> 20<?=$year?></h4></center>
            </div>
            <div class="col-sm-2" align="right">
                <a class="btn btn-primary" href="/dashboard/admin/bronze-mic/<?=$nyr?>/<?=$nm?>">Next Month <i
                ...
                <tr>
                    ...
                    <td>
                        @if($winner == null)
                            ...
                            @csrf
                            <button action="submit" class="btn btn-success btn-simple-tooltip" data-toggle="tooltip"
                                    title="Set as Winner for <?=$mname?>"><i class="fas fa-check"></i></button>
                            {!! Form::close() !!}
                        @elseif($winner->controller_id == $h->id)
                            <a href="/dashboard/admin/bronze-mic/remove/{{ $winner->id }}/{{ $year }}/{{ $month }}"
                               class="btn btn-danger btn-simple-tooltip" data-toggle="tooltip" title="Remove Winner"><i
